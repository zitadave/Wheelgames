...
const target = `            if (item) {
              return (
                ...
                >
                  <span className={\`text-[11px] font-black font-mono leading-none z-10 \${isWinner || item.isSelf ? 'text-white' : 'text-gray-800 dark:text-gray-400'}\`}>{num}</span>
                  <div className={\`flex items-center justify-center mt-0.5 z-10 \${item.isSelf ? '' : 'opacity-60'}\`}>
                    {item.isSelf ? (
                      ...
                      <Lock className={\`w-2 h-2 shrink-0 \${isWinner ? 'text-white' : 'text-gray-500'}\`} />
                    )}
                  </div>
                </motion.div>
              );
